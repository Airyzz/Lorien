...
static func save_project(project: Project) -> void:
	...
	var err = file.open_compressed(project.filepath, FileAccess.WRITE, COMPRESSION_METHOD)
	if err != OK:
...
static func load_project(project: Project) -> void:
	...
	# Open file
	var file := FileAccess.new()
	var err = file.open_compressed(project.filepath, FileAccess.READ, COMPRESSION_METHOD)
	if err != OK:
